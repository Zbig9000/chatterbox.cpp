...
static void stage_G2(const model_ctx & m, const std::string & ref_dir) {
    ...
    // QVAC-17872 round-HIFT/G2-fix: mark xc as graph output so the
    // gallocator preserves its buffer across compute (otherwise the
    // intermediate is overwritten and the diagnostic read of xc returns
    // garbage).  cfm_concat.npy is now produced by dump-s3gen-reference.py
    // (round-HIFT G2-gap closure).
